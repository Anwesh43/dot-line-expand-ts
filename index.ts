...
class DLENode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new DLENode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawLDENode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : DLENode {
        var curr : DLENode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
